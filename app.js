...
function pCard(l){
  ...
  // ── Image section ──
  var imgHTML='<div class="pc-no-img"><svg class="icn icn-sm" aria-hidden="true" style="vertical-align:-3px;"><use href="#i-home"/></svg></div>';
  if(hasImgs){
    ...
    if(showLock){
      imgHTML+='<div class="pc-blur">'
        +'<div style="font-size:26px;"><svg class="icn icn-sm" aria-hidden="true" style="vertical-align:-3px;"><use href="#i-lock"/></svg></div>'
    ...
    if(!ir){
      ...
      if(l.rera)statusLeft+=(statusLeft?' &middot; ':'')+('<span style="color:#7fffb0;">RERA <svg class="icn icn-sm" aria-hidden="true" style="vertical-align:-3px;"><use href="#i-check"/></svg></span>');
    } else {
    ...
    imgHTML+='<div class="pc-status">'+(statusLeft?'<span class="pc-avail">'+statusLeft+'</span>':'')+'<span class="pc-cnt"><svg class="icn icn-sm" aria-hidden="true" style="vertical-align:-3px;"><use href="#i-eye"/></svg> '+visCount+'/'+l.images.length+'</span></div>';
  }

  // ── Inline badge ──
  var badge='';
  if(!ir&&l.stype==='New')badge='<span class="pc-badge new">New Booking</span>';
  else if(l.verified)badge='<span class="pc-badge verified"><svg class="icn icn-sm" aria-hidden="true" style="vertical-align:-3px;"><use href="#i-check"/></svg> Verified</span>';
  ...
  return '<div class="pc" onclick="viewL('+l.id+')">'
    ...
    +'<button class="fave-btn" onclick="event.stopPropagation();togFav('+l.id+',this)" aria-label="Save to favorites" aria-pressed="'+(fv?'true':'false')+'" style="color:'+(fv?'#d9534f':'#fff')+';"><svg class="icn icn-sm" aria-hidden="true"><use href="#'+(fv?'i-heart-fill':'i-heart')+'"/></svg></button>'
    +imgHTML
    +'</div>'
    // Body
    +'<div class="pc-body">'
    +'<div class="pc-title-row"><div class="pt">'+esc(l.title)+'</div>'+badge+'</div>'
    +'<div class="ploc"><svg class="icn icn-sm" aria-hidden="true" style="vertical-align:-3px;"><use href="#i-pin"/></svg> '+esc(l.loc)+', '+esc(l.city)+'</div>'
    +variantsHTML
